...
class WOFPlayer():
    ...
    def addMoney(self, amt):
        self.prizeMoney += amt  # amt = amount

    def goBankrupt(self):
        self.prizeMoney = 0  # perdeu tudo (na rodada)

    def addPrize(self, prize):
        self.prizes.append(prize)  # adiciona no vetor aquilo que o player ganhou naquela rodada
...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        Guess a letter, phrase or type 'exit' or 'pass':'''.format(self.name, self.prizeMoney, category, obscuredPhrase,
                                                                   guessed)
        return response
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self):
        ...
        if move > self.difficulty:
            return True
        else:
            return False

    def getPossibleLetters(self, guessed):
        response = [l for l in self.SORTED_FREQUENCIES if l not in guessed]
        if self.prizeMoney < 250:
            response = list(filter(lambda l: l not in self.VOWELS, response))
